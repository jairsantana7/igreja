...
ALTER TABLE public.events
  ADD COLUMN pix_integration_id uuid,
  ADD CONSTRAINT events_pix_integration_tenant_fk
    FOREIGN KEY (pix_integration_id, tenant_id)
    REFERENCES public.community_integrations (id, tenant_id)
    ON DELETE SET NULL (pix_integration_id);

CREATE INDEX events_tenant_pix_integration_idx
  ON public.events (tenant_id, pix_integration_id)
  WHERE pix_integration_id IS NOT NULL;

ALTER TABLE public.event_registrations
  ADD COLUMN pix_integration_id uuid,
  ADD COLUMN pix_payment_amount_cents integer,
  ADD COLUMN pix_payment_declared_at timestamptz,
  ADD CONSTRAINT event_registrations_pix_integration_tenant_fk
    FOREIGN KEY (pix_integration_id, tenant_id)
    REFERENCES public.community_integrations (id, tenant_id)
    ON DELETE RESTRICT,
  ADD CONSTRAINT event_registrations_pix_declaration_check CHECK (
    (pix_integration_id IS NULL AND pix_payment_amount_cents IS NULL AND pix_payment_declared_at IS NULL)
    OR
    (pix_integration_id IS NOT NULL AND pix_payment_amount_cents > 0 AND pix_payment_declared_at IS NOT NULL)
  );

CREATE INDEX event_registrations_tenant_pix_declared_idx
  ON public.event_registrations (tenant_id, event_id, pix_payment_declared_at, id)
  WHERE pix_payment_declared_at IS NOT NULL;
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'pix', (
      ...
      WHERE integrations.id = events.pix_integration_id
        AND integrations.tenant_id = events.tenant_id
