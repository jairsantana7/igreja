...
interface Capabilities { canManage: boolean; canDelete: boolean; canManageNotes: boolean; canManagePipeline: boolean }
...
const { data: capabilities } = await useAsyncData('followup-capabilities', () => api<Capabilities>('/followups/capabilities'), { server: false });
const canManage = computed(() => capabilities.value?.canManage ?? permissions.value.includes('followups.manage'));
const canDelete = computed(() => capabilities.value?.canDelete ?? permissions.value.includes('followups.delete'));
const canManageNotes = computed(() => capabilities.value?.canManageNotes ?? permissions.value.includes('followups.notes_manage'));
const canManagePipeline = computed(() => capabilities.value?.canManagePipeline ?? permissions.value.includes('followups.pipeline_manage'));
...
const feedback = ref('');
const followupToDelete = ref<Card | null>(null);
const query = ref('');
