    ...
    <section class="section-block">
      ...
      <div v-else class="event-list">
        <article v-for="event in data.events" :key="event.id" class="event-card">
          ...
          <div class="event-card__meta"><strong>{{ event.registrations }}</strong><small>inscrições · {{ event.attendance }} presenças</small><div class="event-card__actions event-card__actions--compact" aria-label="Ações do evento"><NuxtLink :to="`/events/${event.id}`" class="event-action-button event-action-button--primary"><span aria-hidden="true">◎</span> Gerenciar</NuxtLink><NuxtLink v-if="event.status === 'published'" :to="`/e/${event.publicId}`" class="event-action-button event-action-button--secondary"><span aria-hidden="true">↗</span> Abrir</NuxtLink></div></div>
        </article>
      </div>
    </section>
  </div>
