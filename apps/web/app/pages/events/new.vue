...
const form = reactive({ title: '', description: '', location: '', startsAt: '', registrationDeadline: '', capacity: undefined as number | undefined, mediaDisplayMode: 'hero' as 'hero' | 'carousel' | 'fixed', linkedGalleryId: '', familyRegistrationEnabled: false, publish: true, fields: [] as any[], offerings: [] as any[] });
  ...
  <div class="page page--narrow">
    ...
    <form class="editor" @submit.prevent="save">
      ...
      <section class="editor-card">
        <div class="editor-card__heading"><span>2</span><div><h2>Imagens do evento</h2><p>Adicione capas ou fotos e escolha como elas aparecem na página pública.</p></div></div>
        <div class="media-editor"><label class="media-upload"><input type="file" accept="image/jpeg,image/png,image/webp" multiple @change="selectImages"><span>＋ Selecionar imagens</span><small>JPEG, PNG ou WebP · até 5 MiB por arquivo</small></label><fieldset class="media-layout-options"><legend>Modo de exibição</legend><label v-for="option in [{ key: 'hero', label: 'Hero', hint: 'Capa ampla' }, { key: 'carousel', label: 'Carrossel', hint: 'Galeria navegável' }, { key: 'fixed', label: 'Fixa', hint: 'Fundo da página' }]" :key="option.key" :class="{ active: form.mediaDisplayMode === option.key }"><input v-model="form.mediaDisplayMode" type="radio" :value="option.key"><strong>{{ option.label }}</strong><small>{{ option.hint }}</small></label></fieldset></div>
        <div v-if="previews.length" class="media-preview-list"><figure v-for="(preview, index) in previews" :key="preview"><img :src="preview" :alt="`Prévia ${index + 1}`"><figcaption>{{ index === 0 ? 'Imagem principal' : `Imagem ${index + 1}` }}</figcaption></figure></div>
        <div v-if="canLinkGallery" class="gallery-link-picker">
          <div><p class="eyebrow">Memórias da comunidade</p><h3>Destacar uma galeria anterior</h3><p>Mostre como foi outro encontro sem copiar ou duplicar as fotos.</p></div>
          <label class="field"><span>Galeria na página pública</span><select v-model="form.linkedGalleryId"><option value="">Não exibir galeria</option><option v-for="gallery in galleryOptions" :key="gallery.id" :value="gallery.id">{{ gallery.title }} — {{ gallery.event.title }} ({{ galleryDate(gallery.event.startsAt) }})</option></select><small>São listadas as galerias públicas e publicadas desta comunidade.</small></label>
          <article v-if="selectedGallery" class="gallery-link-preview"><img v-if="galleryCoverUrl(selectedGallery)" :src="galleryCoverUrl(selectedGallery)" :alt="selectedGallery.title"><div><strong>{{ selectedGallery.title }}</strong><span>{{ selectedGallery.event.title }} · {{ selectedGallery.photoCount }} {{ selectedGallery.photoCount === 1 ? 'foto' : 'fotos' }}</span></div></article>
        </div>
      </section>
