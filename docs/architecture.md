...
- **Cadastro progressivo e inscrições:** a identidade continua individual; o perfil complementar fornece dados reutilizáveis e a inscrição registra fotografias dos participantes e dos adicionais escolhidos. O PIX manual registra somente uma autodeclaração; pagamento conciliado permanece atrás de `PaymentGateway` e não é inferido de uma seleção.
...
- **WhatsApp Web:** `whatsapp_web` é uma capacidade do canal. Drivers como Baileys vivem em `infrastructure`, guardam estado opaco por uma porta criptografada e executam em worker separado. A API apenas autoriza o pareamento e enfileira comandos.
...
- **Pagamentos:** permanece separado de inscrição. O BR Code estático é uma instrução manual, enquanto `PaymentGateway` evita dependência de fornecedor para futuras cobranças conciliadas, regras de reembolso e webhooks.
